  ...
  <script>
    ...
    // These arrays are used for demo mode. Replace loads with real API calls.
    let DEMO = {
      companies: [],
      ipos: [],
      investors: [],
    ...
    function loadAllForDemo(){
      ...
      DEMO.companies = [
        ...
        {CompanyID:5, CompanyName:'HDFC Bank', Sector:'Finance', CEO:'Vikram Singh', Headquarters:'New Delhi'},
    ...
    function saveCompany(){
      ...
      if(id){
        ...
        // create demo
        const newId = Math.max(...DEMO.companies.map(x=>x.CompanyID))+1;
    ...
    function openIPOForm(){
      // For brevity, show a prompt - in real app open a proper modal
      ...
      if(!cid) return;
      const nextId = Math.max(...DEMO.ipos.map(x=>x.IPOID))+1;
      DEMO.ipos.push({IPOID:nextId, CompanyID:cid, CompanyName:DEMO.companies.find(c=>c.CompanyID===cid).CompanyName, IssueDate:new Date().toISOString().split('T')[0], PricePerShare:150, TotalShares:100000, Status:'OPEN'});
    ...
    function loadApplications(){
      ...
      // populate selects for new application modal
    ...
    function saveApplication(){
      ...
      const newId = Math.max(...DEMO.applications.map(x=>x.ApplicationID))+1;
    ...
    function saveTransaction(){
      ...
      const newId = Math.max(...DEMO.transactions.map(x=>x.TransactionID))+1;
    ...
    function globalSearch(e){
      ...
      // filter companies table quickly
      const tbody = document.querySelector('#table-companies tbody');
      tbody.innerHTML = '';
